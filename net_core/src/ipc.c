...
#define RPMSG_RX_STACK_SIZE     1024
#define RPMSG_RX_PRIO           8

static K_KERNEL_STACK_DEFINE(rpmsg_rx_thread_stack, RPMSG_RX_STACK_SIZE);
static struct k_thread rpmsg_rx_thread_data;
...
static void rpmsg_rx_thread(void *p1, void *p2, void *p3)
{
    ARG_UNUSED(p1);
    ARG_UNUSED(p2);
    ARG_UNUSED(p3);
...
int rpmsg_platform_init(void)
{
    ...
    k_thread_create(&rpmsg_rx_thread_data, rpmsg_rx_thread_stack,
            K_KERNEL_STACK_SIZEOF(rpmsg_rx_thread_stack),
            rpmsg_rx_thread, NULL, NULL, NULL,
            K_PRIO_COOP(RPMSG_RX_PRIO),
            0, K_NO_WAIT);
...
int rpmsg_platform_send(struct net_buf_simple *buf)
{
